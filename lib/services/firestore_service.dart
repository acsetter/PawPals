...
class FirestoreService {
  ...
  static DocumentReference<UserModel> get _userRef =>
      _users.doc(_uid)
          .withConverter(
  ...
  static DocumentReference<PreferencesModel> get _prefRef =>
      _prefs.doc(_uid)
          .withConverter(
  ...
  static Stream<List<PostModel>?> get appUserPostsStream =>
      _posts
          ...
          .snapshots()
          .map((snapshot) =>
      List<PostModel>.from(snapshot.docs.map((doc) => doc.data())))
          .asBroadcastStream();

  /// A broadcast stream that notifies listeners when the list of Posts that
  /// belong to the 'uid' of the expected [UserModel] updates/changes.
  static Stream<List<PostModel>?> userPostsStream(UserModel userModel) =>
      _posts
  ...
  static Stream<List<PostModel>?> userLikedPostsStream(UserModel userModel) =>
      _posts
  ...
  static Future<bool> createUser(UserModel userModel) async {
    ...
    if (userModel.uid != _uid) {
      Logger.log("User being created does not match the user logged in",
          isError: true);
      return false;
    }
    // Set the timestamp to the moment the account was created.
    userModel.timestamp = DateTime
        .now()
  ...
  static Future<bool> updateUser(
      {required UserModel userModel, File? file}) async {
    // Restrict creating/overwriting if not authorized.
    if (_uid == null) {
    ...
    if (userModel.uid != _uid) {
      Logger.log("User being updated does not match the user logged in",
          isError: true);
      return false;
    }

    return await _userRef
        .update(userModel.copyWith(photoUrl: file != null
        ? await StorageService.uploadProfileImage(file)
        : null)
        .toFirestoreUpdate())
        .then((res) {
  ...
  static Future<bool> createPost(PostModel postModel, File file) async {
    ...
    if (newPost.petPhotoUrl == null) {
      Logger.log("Unable to upload post image; aborting post creation.",
          isError: true);
      return false;
    }
  ...
  static Future<List<PostModel>?> getPostsFromIds(List<String>? idList) async {
    return await _posts
        ...
        .then(
            (value) =>
        List<PostModel>.from(value.docs.map((snapshot) => snapshot.data())),
        onError: (e) => Logger.log(e.toString(), isError: true));
  }
  ...
  static Future<List<PostModel>?> getPostsByUser(String uid) async {
    return await _posts
        ...
        .then(
            (snapshot) =>
        List<PostModel>.from(snapshot.docs.map((snapshot) => snapshot.data())),
        onError: (e) => Logger.log(e.toString(), isError: true));
  }

  /// Gets a list of [PostModel]s from an expected [UserModel] from the database.
  static Future<List<PostModel>?> likedPostsByUser(UserModel userModel) =>
      _posts
          .where("postId", whereIn: userModel.likedPosts ?? ["_"])
          .withConverter(
          fromFirestore: PostModel.fromFirestore,
  ...
  /// Returns an error if the post uid does not match the uid of the logged-in user.
  static Future<bool> deletePost(PostModel postModel) async {

    if (postModel.uid != _uid) {
      Logger.log("Post uid does not correspond to uid. ",
          isError: true);
      return false;
    }
